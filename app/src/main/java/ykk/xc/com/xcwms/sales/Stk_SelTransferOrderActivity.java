...
import ykk.xc.com.xcwms.sales.adapter.Stk_SelTransferOrderAdapter;
...
public class Stk_SelTransferOrderActivity extends BaseActivity implements XRecyclerView.LoadingListener {
    ...
    private Stk_SelTransferOrderActivity context = this;
    private static final int SUCC1 = 200, UNSUCC1 = 500;
//    private Supplier supplier; // 供应商
    private OkHttpClient okHttpClient = new OkHttpClient();
    private Stk_SelTransferOrderAdapter mAdapter;
    private List<StkTransferApp> listDatas = new ArrayList<>();
    private List<StkTransferApp> sourceList; // 上个界面传来的数据列表
    private int limit = 1;
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<Stk_SelTransferOrderActivity> mActivity;

        public MyHandler(Stk_SelTransferOrderActivity activity) {
            mActivity = new WeakReference<Stk_SelTransferOrderActivity>(activity);
        }

        public void handleMessage(Message msg) {
            Stk_SelTransferOrderActivity m = mActivity.get();
            if (m != null) {
                m.hideLoadDialog();

                switch (msg.what) {
                    case SUCC1: // 成功
                        List<StkTransferApp> list = JsonUtil.strToList2((String) msg.obj, StkTransferApp.class);
    ...
    public int setLayoutResID() {
        return R.layout.stk_sel_transfer_order;
    }
    ...
    public void initView() {
        ...
        mAdapter = new Stk_SelTransferOrderAdapter(context, listDatas);
        ...
        mAdapter.setOnItemClickListener(new BaseRecyclerAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseRecyclerAdapter adapter, BaseRecyclerAdapter.RecyclerHolder holder, View view, int pos) {
                StkTransferApp recOrder = listDatas.get(pos-1);
                ...
                for(int i=0; i<size; i++) {
                    StkTransferApp recOrder2 = listDatas.get(i);
...
//            sourceList = (List<StkTransferApp>) bundle.getSerializable("sourceList");
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_confirm: // 确认
                ...
                List<StkTransferApp> list = new ArrayList<>();
                for(int i = 0, size = listDatas.size(); i<size; i++) {
                    StkTransferApp p = listDatas.get(i);
                    ...
                    if(p.getIsCheck() == 1) {
                        if(sourceList != null) {
                            for (int j = 0; j < sourceList.size(); j++) {
                                StkTransferApp purOrder2 = sourceList.get(j);
                                // 如果已经选择了相同的行，就提示
                                if (p.getfId() == purOrder2.getfId() && p.getfMaterialId() == purOrder2.getfMaterialId() && p.getfEntryId() == purOrder2.getfEntryId()) {
...
//                    }
    ...
    private void run_okhttpDatas() {
        showLoadDialog("加载中...");
        String mUrl = getURL("stkTransferApp/findByParam");
...
//                .add("supplierId", String.valueOf(supplier.getFsupplierid()))
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                Log.e("Stk_SelTransferOrderActivity --> onResponse", result);
